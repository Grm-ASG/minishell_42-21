...
/*   Updated: 2020/11/28 16:10:28 by imedgar          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void		ft_type_promt(char *envp[])
{
	...
	ft_dprintf(1, "%s%s%s%s:", GREEN, username, promt_line, DEFLT);
	if (pwd_val && home_dir)
	{
		ft_tilda_change((char **)&home_dir, (char **)&pwd_val);
		ft_dprintf(1, "%s%s%s$ ", BLUE, pwd_val, DEFLT);
	}
	else
		ft_dprintf(1, "%s.%s$ ", BLUE, DEFLT);
